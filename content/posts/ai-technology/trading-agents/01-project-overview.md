...
> 图解说明：该图展示项目的来源驱动力（原始项目、中文市场需求、国产大模型生态）如何汇聚成 TradingAgents-CN，并衍生出四大核心目标，确立项目使命与范围边界。
...
> 图解说明：时间轴分四个阶段（基础 → 中文化 → 生态扩展 → 企业级），刻画功能、模型适配与工程能力的递进，帮助评估当前版本所处成熟度层级。
...
> 图解说明：代码主体集中在 Python 核心逻辑（85%），说明项目以后端与智能体编排为重；配置/脚本、文档、前端层比例较低，后续可针对文档自动化与前端交互适度增强。
...
> 图解说明：分层展示从交互（UI/CLI/API）到智能体、模型适配、数据层再到底座基础设施的依赖路径，体现“自上而下依赖、自下而上支撑”的解耦设计。
...
> 图解说明：思维导图聚合项目差异化功能簇：多智能体协作 + 中文金融生态 + 多LLM接入 + 企业级特性，突出与通用英文交易框架的定位差异。
...
> 图解说明：左侧用户类型与右侧使用场景的多对多映射，表明同一核心能力可复用到投资、风险、研究与开发集成等多元业务价值链。
...
> 图解说明：商业/技术/社会价值三层逐级传导，展示技术资产（多智能体、LLM接口、中文NLP）如何放大商业效率并反馈为生态贡献。
...
> 图解说明：路线图采用“3 个月短期 / 6 个月中期 / 12+ 个月长期”分层，以并行轨迹管理功能完善、生态扩展与商业化战略，利于对齐资源规划。
...
> 图解说明：雷达图揭示“测试覆盖率/性能”是下一阶段主要改进点，而功能、文档、维护性已具备较高成熟度，可指导质量投资优先级。
...
> 图解说明：优势链路图显示核心竞争力（中文适配、多智能体、开源生态、部署能力）经技术护城河放大为生态势能，形成正向循环的增长飞轮。
